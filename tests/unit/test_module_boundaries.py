...
@pytest.mark.parametrize(
    "analyze", [extract_information, analyze_sentiment, analyze_text]
)
...
def test_pipeline_orders_items_and_diagnostics_by_source_position():
    ...
    assert [
        (diagnostic.type, diagnostic.reason) for diagnostic in result.diagnostics
    ] == [
    ...
    for records in (result.items, result.diagnostics):
        positions = [record.start for record in records]
        assert positions == sorted(positions)
        for record in records:
            assert text[record.start : record.end] == record.raw
