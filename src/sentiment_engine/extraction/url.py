...
def extract_urls(text: str) -> tuple[list[ExtractionItem], list[Diagnostic]]:
    ...
    for match in URL_CANDIDATE_PATTERN.finditer(text):
        ...
        try:
            ...
            diagnostics.append(Diagnostic("url", raw, start, end, "invalid_url"))
            continue
        if parsed.scheme.lower() not in {"http", "https"}:
            continue
        if not hostname:
            diagnostics.append(Diagnostic("url", raw, start, end, "missing_url_host"))
            continue
        ...
        items.append(ExtractionItem("url", raw, normalized, start, end))
...
def _trim_url_candidate(candidate: str) -> str:
    while candidate:
        ...
        closing = candidate[-1]
        pairs = {")": "(", "]": "[", "}": "{", ">": "<"}
        if closing in pairs and candidate.count(closing) > candidate.count(
            pairs[closing]
        ):
