...
MONEY_TOKEN_PATTERN = re.compile(
    r"(?P<number>\d[\d,]*)(?:\s*(?P<unit>억|천만|만|천))?\s*"
)
...
def extract_money(text: str) -> tuple[list[ExtractionItem], list[Diagnostic]]:
    ...
    for match in USD_CANDIDATE_PATTERN.finditer(text):
        ...
        if not INTEGER_PATTERN.fullmatch(number):
            diagnostics.append(
                Diagnostic(
                    "money", raw, match.start(), match.end(), "invalid_money_number"
                )
            )
            continue
        amount = int(number.replace(",", ""))
        normalized = MoneyValue(amount, "USD")
        items.append(
            ExtractionItem("money", raw, normalized, match.start(), match.end())
        )
    for match in KRW_CANDIDATE_PATTERN.finditer(text):
        ...
        if reason:
            diagnostics.append(
                Diagnostic("money", raw, match.start(), match.end(), reason)
            )
            continue
        normalized = MoneyValue(value, "KRW")
        items.append(
            ExtractionItem("money", raw, normalized, match.start(), match.end())
        )
