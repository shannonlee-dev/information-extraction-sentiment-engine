...
def main(argv: list[str] | None = None):
    ...
    output.add_argument(
        "--no-save", action="store_true", help="파일 저장 없이 터미널에만 출력"
    )
    parser.add_argument(
        "--format",
    ...
    evaluation = args.evaluate is not None
    text_output = args.format == "text"
    if args.format == "auto":
        text_output = sys.stdout.isatty()
    if text_output:
        print(terminal_report(result, evaluation=evaluation), flush=True)
    else:
        print(json.dumps(result, ensure_ascii=False, indent=2), flush=True)
    if not args.no_save:
        try:
            directory = save_artifacts(result, args.output_dir, evaluation=evaluation)
            ...
            print(
                "평가·분석 결과는 위에 출력되었습니다. "
                "파일 저장이 필요 없으면 --no-save를 사용하세요.",
                file=sys.stderr,
            )
            raise SystemExit(1) from error
        print(f"\n저장 완료  {directory}", file=sys.stderr)
        saved_paths = sorted(directory.iterdir())
        saved_names = [path.name for path in saved_paths]
        print("  " + " · ".join(saved_names), file=sys.stderr)
