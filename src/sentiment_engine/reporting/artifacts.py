...
def save_artifacts(result: dict, output_dir: Path, *, evaluation: bool) -> Path:
    ...
    if evaluation:
        if "extraction" in result and "sentiment" in result:
            mode = "all"
        ...
        directory = Path(
            mkdtemp(prefix=f"analysis-{timestamp}-", dir=output_dir)
        ).resolve()
    ...
    summary.write_text(
        summary_markdown(result, evaluation=evaluation), encoding="utf-8"
    )
    if evaluation and "sentiment" in result:
        ...
        summary.write_text(
            summary_markdown(result, evaluation=True, chart=True), encoding="utf-8"
        )
    return directory
