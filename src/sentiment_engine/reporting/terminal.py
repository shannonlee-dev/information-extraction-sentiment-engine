...
def terminal_report(result: dict, *, evaluation: bool) -> str:
    ...
    if not evaluation:
        ...
        if result["diagnostics"]:
            lines.append(
                f"확인 필요  {len(result['diagnostics'])}건 (상세 내용은 JSON 참조)"
            )
    else:
        if "extraction" in result:
            ...
            score_rows = list(report["per_type"].items())
            score_rows.append(("micro", report["micro"]))
            for kind, scores in score_rows:
                lines.append(
                    f"  {kind:<10} {scores['precision']:>10.4f} "
        ...
        if "sentiment" in result:
            ...
            lines += [
                "",
                f"감성 분석 · {sample_count(comparison)}문장",
