...
def _text(value: object) -> str:
    ...
    return ''.join(f'&#{ord(char)};' if char in r'\`*_{}[]()#+-.!|>' else
                   html.escape(char) for char in text)
...
def summary_markdown(result: dict, *, evaluation: bool, chart: bool = False) -> str:
    ...
    if not evaluation:
        ...
        if sentiment['matches']:
            ...
            lines += ['', '매칭 순서대로 최대 5개를 표시합니다. 점수는 확률이 아닙니다.']
        if result['diagnostics']:
            lines += ['', f"유효성 검사에서 제외된 후보: {len(result['diagnostics'])}건. "
                      '상세 사유는 전체 데이터를 확인하세요.']
    else:
        if 'sentiment' in result:
            comparison = result['sentiment']
            off, on = comparison['without_modifiers'], comparison['with_modifiers']
            lines += ['## 감성 분석', '',
                      f"**{sample_count(comparison)}문장 중 "
                      f"{sample_count(comparison) - len(on['errors'])}문장 정답** · "
...
def _errors(errors: list[dict], *, sentiment: bool) -> list[str]:
    ...
    for error in errors[:3]:
        detail = (f"{LABELS[error['expected']]} → {LABELS[error['predicted']]}" if sentiment
                  else {'fn': '누락', 'fp': '추가 추출'}[error['kind']])
        lines.append(f"| {_text(error['case_id'])} | {_text(error['text'])} | {detail} |")
    return lines
