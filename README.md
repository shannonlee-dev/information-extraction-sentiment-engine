...
- 한국어 활용형 매칭, 방향을 구분한 부정어 연결, 강조·이중부정 계산 근거 제공
- 두 고정 합성 평가에서 정확도 92% / 90%, 단위·통합·성능 회귀 검증
...
├── sentiment.py + korean.py: 토큰화 → 활용형 포함 최장 일치 → 수정어 연결 → 합산
...
| `src/sentiment_engine/sentiment.py` | 자원 검증, 최장 일치, 수정어 연결, 점수 계산 |
| `src/sentiment_engine/korean.py` | 조사·어미의 유한 표면형 생성, 규칙과 캐시 |
...
`sentiment_lexicon.json`에는 고유 대표어 207개가 있다. 양수 107개, 음수 100개이며 점수는 `-3, -2, -1, 1, 2, 3` 중 하나다. 절댓값 1은 약한 감성, 2는 명확한 감성, 3은 매우 강한 감성이며 부호가 극성을 나타낸다. 이 중 `customer_support` 도메인 표현은 31개로 배송, 응대, 환불, 교환, 결제, 품질, 대기, 문제 해결을 포함한다. 각 항목은 대표어 `term`, 표면형 `variants`, `score`, 선택적 `domain`, `source`를 가진다.
...
토큰화 정규식은 연속된 한글·영문·숫자를 어절 하나로 묶고 `.!?,;:`를 경계 토큰으로 남긴다. 원문 토큰을 변형하지 않고, 사전 쪽 표면형을 `korean.py`에서 유한하게 확장한다. 명사는 `불만이`, `고장으로`, `최고네요` 같은 조사·서술격 형태를, 용언은 `좋고`, `친절하지`, `추천합니다` 같은 어미와 과거형을 인식한다. `하/해`, `되/돼`는 명시적 규칙을 쓰고, `부드러워 → 부드러웠어요` 같은 불규칙 활용은 등록된 표면형을 기반으로 한다. 명사 대표어 `피해`를 동사 `피했다`로 활용하지 않도록 명사와 제공된 활용형을 구분한다. 완전한 형태소 분석기는 아니며 생성하지 않은 활용은 놓칠 수 있다.

생성된 **어절 전체**가 일치해야 하므로 `문제집`, `최고기온`, `고장난감` 같은 임의 부분 문자열을 감성으로 잡지 않는다. 여러 어절 표현은 마지막 어절을 활용하며 최장 표현이 겹치는 짧은 표현을 차지한다. 명시적으로 등록된 표면형을 생성형보다 우선하고, 생성형끼리 점수가 충돌하면 그 생성형은 제외한다. 인덱스는 캐시하며 `raw`, `start`, `end`는 입력의 공백까지 보존한다.
...
강조어는 같은 구간에서 뒤에 나오는 첫 감성 표현에만 연결되고, 사이의 단어가 최대 2개일 때 적용된다. 여러 강조 배수는 곱하되 2.0으로 제한한다. 문장부호, `하지만/그러나/그런데/그래도/반면/그리고`, `-지만/-는데/-은데`는 수정어가 다음 절로 넘어가지 않게 한다.

부정어 연결 정책은 다음과 같다.

- `안/못` 단독형은 뒤의 감성 표현, `않다/없다`와 `못하다/못해요` 같은 후행 보조용언은 앞의 감성 표현에 연결한다. `아니다`는 앞뒤의 가장 가까운 표현을 선택하며 거리가 같으면 뒤쪽을 택한다. 직접 연결은 사이 단어 최대 2개다.
- `도움이 안 된다`, `만족 안 해요`, `정확하다고 못 하겠습니다`처럼 명사·인용 표현 뒤의 가벼운 서술어에 부정이 붙는 제한된 구성은 앞 감성 표현에 연결한다.
- 이미 연결한 부정어 뒤에 `것은`, `이유가`, `수가`, `할 수는` 등의 정해진 연결 구문이 나오면 같은 감성 표현의 부정 횟수를 증가시킨다. 다른 감성 표현이나 절 경계를 건너지 않는다. 따라서 `좋지 않은 것은 아니다`, `나쁘지 않다고 할 수는 없다`를 처리한다.
- `해결 안 됨`, `맘에 안 들어`처럼 사전 표현 내부에 포함된 부정어는 다시 반전하지 않는다. `피해 없어`의 사전 점수는 +1로 바로잡았고, `문제가 해결되다`는 해결 의미의 최장 표현으로 등록해 `문제 -2`와 `해결 +2`가 잘못 상쇄되지 않게 했다.

총점이 `> 0`이면 `positive`, `< 0`이면 `negative`, 정확히 `0`이면 `neutral`이다. `mixed=true`는 수정 후 기여도 중 양수와 음수가 모두 존재한다는 뜻이며 최종 레이블과 독립적이다. 단순 합산을 유지하므로 마지막 절을 무조건 우선하지 않는다.

| 예 | 계산 | 결과 |
|---|---|---|
| `정말 좋다` | `2 × 1.5` | `3.0`, positive |
| `정말 유용하지 않았습니다` | `2 × 1.5 × -1` | `-3.0`, negative |
| `만족하지 않는 것은 아니다` | `2 × (-1)^2` | `2.0`, positive |
| `나쁘지 않다 좋다` | `(-2 × -1) + 2` | `4.0`, positive |
| `해결 안 됨` | 사전에 등록된 `-3`, 내부 `안` 재반전 제외 | `-3.0`, negative |

`apply_modifiers=False`는 같은 활용형 매칭에 강조 배수 1.0과 부정 수 0을 사용한다. 따라서 현재 구현에서 수정어 규칙 자체의 영향을 같은 gold 데이터로 비교한다. 과거 구현과의 비교와는 별도 실험이다.
...
기존 구현(`4a6f8d9`)을 같은 기존 100문항에서 재현한 결과는 수정어 끔 Accuracy `0.43` / Macro F1 `0.588357`, 켬 Accuracy `0.44` / Macro F1 `0.602477`이었다. 수정 후 수정어 켬 정확도는 **44% → 92%, +48%p**이며 중립 예측은 **54 → 7건**으로 줄었다. 당시 낮은 성능의 큰 원인은 활용형 미인식과 부정어 연결 오류였다. 현재 수정어 자체의 정확도 효과는 기존 데이터에서 `70% → 92% (+22%p)`, 추가 데이터에서 `70% → 90% (+20%p)`다.
...
추가 데이터 `sentiment_validation_cases.json`은 구현 전에 문장과 레이블을 고정한 AI 보조 합성 자료다. 기존 문장을 복사하지 않고 다른 어휘·높임말·과거형을 포함하지만, 같은 작성 주체와 문법 유형을 공유하므로 독립적인 블라인드 평가나 실제 고객 분포 검증으로 보지 않는다. 구 구현에서는 추가 데이터 정확도가 7%였다. 두 데이터 모두 이번 작업 중 문장·레이블을 바꾸지 않았다. 작업 합격선은 두 데이터 각각 Accuracy와 Macro F1 0.80 이상으로 두었으며, 이를 회귀 테스트로 유지한다.
...
감성 사전에서는 서로 다른 대표어가 같은 variant를 갖거나 문맥에 따라 polarity가 바뀌는 충돌이 발생한다. 로더는 명시적 중복 표면형을 거부하고 생성형의 점수 충돌은 매칭에서 제외하지만, 문맥에 따른 의미 충돌 자체는 판단하지 못한다. 변경 제안에 대표어·표면형·점수·근거·영향 사례를 함께 기록하고, 도메인별 회귀 지표를 비교해야 한다.
...
활용형은 이제 `korean.py`의 제한된 생성 규칙을 공유한다. 새 의미는 사전 항목으로, 불규칙 어간은 `variants`로, 여러 단어에 공통인 조사·어미는 생성 규칙으로 확장한다. 허용 어미를 늘리면 비문법적 생성형이나 동형이의어 오탐도 늘 수 있으므로 `문제집/최고기온` 같은 중립 반례와 원문 범위 회귀를 함께 검증한다. 형태소 분석기를 도입한다면 의존성·버전·오분석을 별도로 검증해야 한다.
